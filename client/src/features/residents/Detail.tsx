          ...
          editBtn={
            resident.ghiChu && (
              <Button
                ...
                ghost
                icon={<EditOutlined />}
                onClick={() => navigate(`/nhan-khau/chinh-sua/${resident._id}`)}
              >
                Chỉnh sửa thông tin giấy khai tử
              </Button>
            )
          }
          deleteBtn={Boolean(resident.duoc_khai_tu) ? <></> : <Death currentResident={resident} />}
        ...
        {Object.keys(resident).length === 0 ? (
          ...
          <>
            <EachResidentInfoDiv label={`Mã nhân khẩu - ${resident.maNhanKhau}`}>
              <div className="flex items-center justify-start gap-8">
                ...
                <div className="grid h-full grow grid-cols-3 gap-4">
                  ...
                  <ResidentInfoItem label="Tuổi" value={resident.age} />
                  {resident.ghiChu && (
                    <ResidentInfoItem
                      label="Ghi chú"
                      value={
            ...
            <EachResidentInfoDiv label="Thông tin căn cước công dân">
              <div className="grid grid-cols-3 gap-4">
                <ResidentInfoItem label="Số căn cước công dân" value={resident.cmt?.soCMT} />
                <ResidentInfoItem
                  label="Ngày cấp"
                  value={
                    resident.cmt?.ngayCap &&
                    new Intl.DateTimeFormat('vi-GB', {
                      year: 'numeric',
                      month: 'long',
                      day: 'numeric'
                    }).format(new Date(resident.cmt.ngayCap))
                  }
                />
                <ResidentInfoItem label="Nơi cấp" value={resident.cmt?.noiCap} />
              </div>
            </EachResidentInfoDiv>
