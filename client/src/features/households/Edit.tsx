...
const initialRelation = (household: IHousehold) => {
  let obj = {
    idChuHo: household.chu_ho._id,
    maHoKhau: household.maHoKhau,
    maKhuVuc: household.maKhuVuc,
    diaChi: household.diaChi
  } as any

  household.thanhVienHo?.forEach(resident => {
    obj[`quanHe-${resident._id}`] = resident?.quanHeVoiChuHo
  })
...
const Edit = () => {
  ...
  const onFinish = async (values: any) => {
    ...
    household.thanhVienHo.forEach(resident => {
      if (resident._id === idChuHo) return
      obj.nhan_khaus.push({
        id: resident._id,
        quanHeVoiChuHo: values[`quanHe-${resident._id}`]
      })
    })
    let error = false
            ...
            <div className="flex justify-between gap-4">
              ...
              >
                <div className="flex justify-between gap-2">
                  ...
                  >
                    <Select
                      showSearch
                      options={household.thanhVienHo.map(resident => ({
                        value: resident._id,
                      ...
                      onSelect={value => {
                        setHousehold({
                          ...household,
                          idChuHo: value,
                          chu_ho: household?.thanhVienHo.find(
                            resident => resident._id === value
                          ) as IResident
                        })
                      }}
                    />
              ...
              <EachHouseholdInfoDiv label="Chủ hộ" className="w-3/5">
                ...
                <div className={`mt-6 rounded-md`}>
                  ...
                  {household.thanhVienHo?.length > 0 ? (
                    <div className="grid max-h-[400px] grid-cols-2 gap-2 overflow-y-auto">
                      {household.thanhVienHo.map(resident => (
                        <div
                          key={resident._id}
                        ...
                        >
                          ...
                          <div className="grid gap-2">
                            ...
                            <div>
                              ...
                              {resident._id === household.chu_ho._id ? (
                                <p className="text-base">Chủ hộ</p>
                              ) : (
                                <Form.Item
                                  name={`quanHe-${resident._id}`}
                                ...
                                >
                                  <Select
                                    ...
                                    onSelect={value => {
                                      setHousehold({
                                        ...household,
                                        thanhVienHo: household.thanhVienHo.map(Tresident => {
                                          if (Tresident._id === resident._id) {
                                            return {
                                              ...Tresident,
