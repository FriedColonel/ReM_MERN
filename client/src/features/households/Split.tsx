...
function SplitHousehold() {
  ...
  const leftColumns: ColumnsType<IResident> = [
    ...
    },
    {
      ...
      dataIndex: 'ngaySinh',
      key: 'ngaySinh',
      render: (_, record) => new Date(record.ngaySinh).toLocaleDateString('vi-VN')
    },
    {
      title: 'Quan hệ với chủ hộ',
      render: (_, record) => record?.quanHeVoiChuHo
    }
  ]
  ...
  const rightColumns: ColumnsType<IResident> = [
    ...
    },
    {
      title: 'Quan hệ với chủ hộ',
      render: (_, record) => {
        if (record._id === newHousehold?.idChuHo) return <p>{'Chủ hộ'}</p>
        return (
          <Select
            ...
            onSelect={value => {
              setNewHousehold({
                ...newHousehold,
                thanhVienHo: newHousehold.thanhVienHo?.map(resident => {
                  if (resident._id === record._id)
                    return {
                      ...resident,
  ...
  useEffect(() => {
    if (household && household._id !== (id as string)) getHouseholdById(id as string)
  }, [household, id])
              ...
              <Table
                ...
                rowSelection={{
                  ...
                  getCheckboxProps: record => ({
                    disabled: record._id === household.chu_ho?._id
                  })
                }}
                columns={leftColumns}
                dataSource={household?.thanhVienHo}
                pagination={false}
              />
            </div>
            <div className="flex w-1/12 items-center justify-center">
              <Tooltip placement="bottom" title={'Tách thành viên'} arrow={false}>
                <Button
                  ...
                  icon={<RightOutlined />}
                  onClick={() => {
                    if (residents.find(resident => resident._id === newHousehold?.idChuHo)) {
                      setNewHousehold({ ...newHousehold, thanhVienHo: residents })
                      return
                    }
                    setNewHousehold({
                      ...newHousehold,
              ...
              <Table
                ...
                rowSelection={{
                  ...
                  onChange: selectedRow => {
                    setNewHousehold({
                      ...newHousehold,
                      idChuHo: selectedRow[0] as string,
                      chu_ho: residents.find(
                        resident => resident._id === selectedRow[0]
                      ) as IResident
                    })
                  },
                ...
                dataSource={newHousehold.thanhVienHo ?? []}
                pagination={false}
              />
            </div>
          </div>
